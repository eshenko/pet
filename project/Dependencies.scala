...
object Dependencies {
  ...
  val dbDependencies = Seq(slick, slickPg, hikariCP, postgresql)
  val coreTestDependencies = Seq(mockito, scalaCheck, scalaTest, scalaMock)
  val allTestDependencies = coreTestDependencies ++ Seq(akkaTestKit, akkaHttpTestKit, logback, wireMock)
  val akkaDependencies = Seq(akkaActor, akkaStreams, akkaHttp)
